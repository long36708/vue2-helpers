...
import { defineComponent, PropType } from 'vue-demi-longmo';
...
test('元素自定义组件兼容', () => {
  ...
  const Native = defineComponent({
    ...
    setup(props, { emit, slots }) {
      expectType<
        ((event: 'foo', args: number) => void) &
          ((event: 'bar', arg: string) => void)
      >(emit);
  ...
  const App = createComponent(() => {
    ...
    t = (
      <Native
        foo="foo"
        onFoo={evt => expectType<number>(evt)}
        onBar={evt => expectType<string>(evt)}
      ></Native>
    );
    ...
    return (
      <div title="app">
        <Native
          foo="foo"
          v-slots={{ before: () => 'before', default: () => 'default' }}
        ></Native>
        <Native foo="foo">
        ...
        <Native
          foo="foo"
          v-slots={{ before: () => 'before', after: () => 'after' }}
        >
          default
        </Native>
      </div>
