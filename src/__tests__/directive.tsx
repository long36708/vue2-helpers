/** @jsxImportSource .. */
import { ref, nextTick } from 'vue-demi-longmo';
  ...
  setup() {
    ...
    return () => (
      <div>
        ...
        <div
          title="innerHTML"
          innerHTML={'<div class="inner">innerHTML</div>'}
        ></div>
        ...
        <input
          ...
          onInput={e => (inputValue.value = (e.target as any).value)}
        />
        ...
        <CustomModel
          modelValue={customInputValue.value}
          onUpdate:modelValue={e => (customInputValue.value = e)}
        />
...
test('Directive', async () => {
  ...
  expect(content.outerHTML).toBe(
    '<span title="content" style="display: none;">show</span>'
  );
  ...
  expect(list.outerHTML).toBe(
    '<div title="list"><div>1</div><div>2</div><div>3</div></div>'
  );
});
