...
import { Fragment, type Teleport, type Suspense } from './components';
...
export function h(type: any, props: any, ...children: any[]): VNode {
  ...
  if (type === Fragment) {
    ...
    if (!isVue2) {
      ...
      if (
        finalProps.directives &&
        isDirectiveArgumentsBinding(finalProps.directives)
      ) {
      ...
      if (directives) {
        vnode = withDirectives(
          vnode,
          directiveBindingToArguments({ directives })
        );
      }
    } else {
      vnode = vueh(type, finalProps, finalChildren);
