...
const ALLOWED_EVENT_MODIFIER = new Set([
  'capture',
  'once',
  'passive',
  'native',
...
export function processVue2Event(
  key: string,
  value: any
): IEventHandler | null {
...
export function vue2GetElementInstance(tag: string) {
  ...
  const isUnknown = normalizedTag.includes('-')
...
export function vue2MustUseProps(
  tag: any,
...
export function processVue2Attr(
  el: { tag: string; type?: string },
  key: string,
  value: any
): IAttr {
  // Vue3 .prop
...
export function mergeProps<T extends {}, S extends {}>(
  target: T,
  source: S
): T & S {
...
export function processChildren(tag: any, props: any, children: any[]): any {
  ...
  const slotsFromChildren =
    children.length === 1 && isSlots(children[0]) ? children[0] : undefined;
  ...
  if (slots || slotsFromChildren) {
    // 显式定义了 slots
    // 数组的话，只能是第一个
    const { scopedSlots, staticSlots } = vue2NormalizeSlots(
      slots || slotsFromChildren
    );
...
export function processRef(tag: any, props: any) {
  ...
  const instance = getCurrentInstance() as unknown as {
    refs: object;
    [VUE2_REFS_CACHE]?: Map<Ref, string>;
  } | null;
  ...
  const cache = (instance[VUE2_REFS_CACHE] =
    instance[VUE2_REFS_CACHE] ?? new Map());
