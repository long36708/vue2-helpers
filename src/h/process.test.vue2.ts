...
import { ref } from 'vue-demi-longmo';
import {
  processChildren,
  isSlots,
  wrap,
...
test('mergeProps', () => {
  ...
  expect(mergeProps({ foo: { bar: 1 } }, { foo: { baz: 2 } })).toEqual({
    foo: { bar: 1, baz: 2 },
  });
  ...
  expect(
    mergeProps({ on: { foo: h1, bar: h1 } }, { on: { foo: h2, baz: h2 } })
  ).toEqual({
...
test('processVue2Attr', () => {
  ...
  // 强制 domProps
  expect(
    processVue2Attr({ tag: 'test', type: 'input' }, '.attr', value)
  ).toEqual({
    domProps: true,
    name: 'attr',
    value,
  ...
  // mustUseProp
  expect(
    processVue2Attr({ tag: 'input', type: 'text' }, 'value', value)
  ).toEqual({
    domProps: true,
    name: 'value',
    value,
  ...
  test('识别 slots', () => {
    {
      // 没有设置 slots
      const props: any = {};
      expect(processChildren('', props, ['hello', 'world'])).toEqual([
        'hello',
        'world',
      ]);
    }
