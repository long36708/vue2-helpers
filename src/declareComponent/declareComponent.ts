 ...
 * @returns
 */
export function declareProps<T extends {}>(
  props: Array<keyof T> | ComponentObjectPropsOptions<Partial<T>>
): T {
  return props as T;
}
 ...
 * @returns
 */
export function declareSlots<T extends Record<string, unknown>>(): {
...
export function declareComponent<
  ...
  Expose extends {} = {},
  Slots extends Record<string, Function> = {},
  Attrs extends {} = {},
>(
  setup: (
    props: Props,
...
export function declareComponent(setUpOrProps: any, maybeOption?: any) {
  const setup =
    typeof setUpOrProps === 'function' ? setUpOrProps : setUpOrProps.setup;
  const options =
    typeof setUpOrProps === 'function' ? maybeOption : setUpOrProps;
    ...
    setup: isVue2
      ? function (props: any, context: any) {
          ...
          const emit = (name: string, ...args: any[]) => {
            ...
            if (found) {
              safeCallHandler(
                instance.proxy.$listeners[found],
